...
printHeader("PixelArt Studio", 
	...
	function postxpm()
	{
		...
		while (child && cild.nodeType != 1) {
			...
			/* convert color strings (e.g. red) to hex */
			if (!o.getPropertyValue("background").toString.match(/^#.+$|^[^0-9]+$/) {
				array[i++] = colorNameToHex(o.getPropertyValue("background"));
			} else {
		...
		var o = getComputedStyle(pixelwindow);
		var w = o.getPropertyValue("width");	
		var h = o.getPropertyValue("height");	

		<? session_start() ?>
		<? $_SESSION['xpmwidth'] = ?> w <? ; ?> 
		<? $_SESSION['xpmheight'] = ?> h <? ; ?> 
		i = 0;
		while (i < array.length) {
			<? $_SESSION['array ?> i.toString() <? '] = array[i];
			i++;
		}
		<? $_SESSION['arraylength'] = ?> i <? ; ?> 
		<? header('Location: pixelart.xpm.php'); ?>
	}	
	</script>" .	
	//CSS link
	"<style>" .
